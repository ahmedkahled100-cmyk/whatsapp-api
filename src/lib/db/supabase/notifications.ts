...
export const addNotification = async (teacherId: string, msg: string, type: Notification['type'] = 'info', targetUsers?: string[], targetRoles?: string[], actionPath?: string) => {
  const data: any = {
...
export const dispatchNotification = async (options: DispatchOptions) => {
  ...
  if (channels.inApp) {
    await addNotification(teacherId, msg, type, targetUsers, targetRoles, actionPath);
...
export const subscribeToNotifications = (teacherId: string, callback: (notifs: Notification[]) => void) => {
  ...
  const fetch = async () => {
    ...
    callback(manyFromDB<Notification>(data || []));
  };

  const channel = supabase
    .channel(`notifs:${teacherId}`)
    .on('postgres_changes', { event: '*', schema: 'public', table: NOTIFICATIONS, filter: `teacher_id=eq.${teacherId}` }, fetch)
    .subscribe();
...
export const subscribeToNotificationLogs = (teacherId: string, callback: (logs: NotificationLog[]) => void) => {
  ...
  const channel = supabase
    ...
    .on('postgres_changes', { event: '*', schema: 'public', table: NOTIFICATION_LOGS, filter: `teacher_id=eq.${teacherId}` }, fetch)
    .subscribe();
  fetch();
  return () => supabase.removeChannel(channel);
};
