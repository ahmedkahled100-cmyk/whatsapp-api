...
import { getTeachers, saveTeacher, deleteTeacher, updateSuperAdminCredentials } from '@/lib/db';
import { TeacherUser } from '@/types';
import { showToast } from '@/lib/toast';
import { UserPlus, Shield, User, Clock, Edit2, Trash2, X, Save, Lock, LayoutDashboard, Bell, FileText, Users, CreditCard, BookMarked, ClipboardList, Calendar, Bot, BarChart2, Check } from 'lucide-react';
...
export default function ManageTeachersPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForm({ name: '', username: '', password: '', code: '', role: 'teacher', permissions: AVAILABLE_PERMISSIONS.map(p => p.id) });
      loadData();
    } catch(err) {
      showToast('حدث خطأ');
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {loading ? <div className="col-span-3 text-center py-20 grayscale opacity-50"><div className="animate-spin text-4xl mb-4">⌛</div>جاري التحميل...</div> : teachers.map(t => (
            <div key={t.id} className="card-base p-5 border border-white/5 hover:border-purple-500/30 transition-all group relative overflow-hidden">
              {/* Bg Glow */}
