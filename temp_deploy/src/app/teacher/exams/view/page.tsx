...
export default function ExamDetailsPage() {
  return (
    <Suspense fallback={<div className="text-center py-20 text-muted">جاري التحميل...</div>}>
      <ExamDetailsContent />
    </Suspense>
  );
}
      ...
      <div className="card-base p-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 pb-6" style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
          ...
          <div className="flex gap-2">
            <Link href={`/teacher/exams/edit?id=${id}`} className="btn-outline text-sm py-2 px-4">
              <Edit size={16} /> تعديل
            </Link>
          </div>
        </div>
